...
run_emma <- function(x){
  ...
  gsea_res <- paste0(subdir, comparison, "_c2.cp.v2023.1.Hs.symbols.gmt_stats.rds")
  df <- readRDS(gsea_res)
  
  outdir <- '/projectnb/tcwlab/LabMember/mwu/Project/Astrocyte_Ab/2X100/output/Pathway/'
  
  #Top 40 pathways in either directions
  activated <- head(df[order(df$NES, decreasing = TRUE), ], 40)
  suppressed <- head(df[order(df$NES, decreasing = FALSE), ], 40)
...
#Example
df <- readRDS("/projectnb/tcwlab/LabMember/mwu/Project/Astrocyte_Ab/2X100/output/GSEA/Degrade_33_24hrvsCtrl/Degrade_33_24hrvsCtrl_c5.go.v2023.1.Hs.symbols.gmt_stats.rds")

df_sorted <- df[order(abs(df$NES), decreasing = TRUE), ]

test <- emmaplot(df_sorted[1:80,], label.size = 3)
ggsave("test.pdf", height = 10, width = 20)
...
plot_WGCNA_threshold <- function() {
  ...
  df_filtered <- df[rowSums(edgeR::cpm(df[,-1])>1)>=8*0.95,]
