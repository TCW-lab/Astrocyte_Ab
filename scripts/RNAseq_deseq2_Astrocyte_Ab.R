...
run_deseq2_analysis_genotype <- function () {
  ...
  for (i in 1:nrow(conf)) {
    ...
    ref_covaA <- ref_cova %>% 
      dplyr::filter(ref_cova$Sample_Name == refSubGroup) %>%
      dplyr::select(all_of(select_cols)) #select_cols are the columns specified in the config file (META COL:)
    
    ref_exprA<- ref_exprs[,colnames(ref_exprs) == as.character(ref_covaA$Sample_Name)]
    
    target_covaA <- target_cova %>% 
      dplyr::filter(ref_cova$Sample_Name == targetSubGroup)%>%
      dplyr::select(all_of(select_cols))
    
    target_exprA<- target_exprs[,colnames(target_exprs) == as.character(target_covaA$Sample_Name)]
      ...
      switch(cmp_cols[j],
             SampleGroup={
               covaA$Sample_Name <- factor(
                 ifelse(covaA$Sample_Name == refSubGroup, ref_str, target_str), 
...
############## Defines the second function for degradation experiment, changing covariate ##############
run_deseq2_analysis_time <- function () {
  ...
  for (i in 1:nrow(conf)) {
    ...
    target_covaA <- target_cova %>% 
      dplyr::filter(target_cova$Sample_Name == targetSubGroup)%>%
      dplyr::select(all_of(select_cols))
...
for (c in c(3,4,8,9)) {
  ...
  meta_dir <- paste0(meta_root, cfg_name, "/"); #results/metadata/config_dir
  ...
  for (i in 1:ncol(ref_exprs)) {
    colnames(ref_exprs)[i] <- substr(colnames(ref_exprs)[i], 5, nchar(colnames(ref_exprs)[i]))
  }
  
  target_exprs <- as.matrix(read.table(target_fc_file, header=TRUE, sep="\t", row.names=1,as.is=TRUE,check.names = FALSE))
  
  for (i in 1:ncol(target_exprs)) {
    colnames(target_exprs)[i] <- substr(colnames(target_exprs)[i], 5, nchar(colnames(target_exprs)[i]))
  }
  ...
  #Change the column names to meet the design for these IDs
  for (i in 1:nrow(ref_cova)) {
    ref_cova[i, 1] <- substr(ref_cova[i, 1], 5, nchar(ref_cova[i, 1]))
  }
  
  for (i in 1:nrow(target_cova)) {
    target_cova[i, 1] <- substr(target_cova[i, 1], 5, nchar(target_cova[i, 1]))
